...
import { PaymentCaptured } from '../../domain/payments/PaymentDomainEvent';
...
export class CapturePaymentUseCase {
  ...
  async execute(command: CapturePaymentCommand): Promise<void> {
    ...
    const captureEvent: PaymentCaptured = {
    ...
    applyPaymentEvent(currentState, captureEvent);

    // Append event
    await this.paymentRepository.appendEvents(command.paymentId, [captureEvent], events.length);
  }
}
